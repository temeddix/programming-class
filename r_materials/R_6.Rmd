...
```{r}
d1<-data.frame(A=letters[1:5], B=1:5,stringsAsFactors=TRUE) #A,B변수 : A는 문자, B는 숫자형. Giving `stringsAsFactors`, it takes strings as factors, whose deafult value is FALSE. 
d1
```
...
```{r}
names(d1)
names(d1)<-c("Var1","Var2")
str(d1)
```

```{r}
names(d1)[2]<-c("B") #2번째 변수 이름만 바꾸기 
str(d1)
```
...
```{r}
d1
d1[1,] #1행 # Row 1, all columns
d1[,1] #1열 ,즉 Var1 변수 전체 # All rows, column 1
...
```{r}
d1$B   #B 변수 전체 
d1[c(1,3,5),] #1,3,5번째 행 
d1[c(1,3,5),1] #1,3,5번째 행의 Var1 변수값  또는, d1$Var1[c(1,3,5)] 
d1[c(1,3,5),"Var1"]
d1$Var1[c(1,3,5)]
```  
...
subset(col_df, col1==4, select=col2)
```

- ex2_2. 위 col_df에서 col1이 4인 경우의 행 전체를 제거하는 프로그램을 작성하라. 

```{r}
col_df<-col_df[col_df$col1!=4,]
col_df[!col_df$col1==4,]
subset(col_df,col1!=4)
```
...
```{r}
col_df[1:3,] # In R, the range end in INCLUSIVE
...
d1$B<-NULL  # d1<-d1[,-2]
d1
d1<-subset(d1, select=-Var1) # Var1 변수 제거
```
...
```{r}
head(cars) # R 내부에 있는 data , speed (of cars) , dist (stopping distance)
head(cars$dist/cars$speed) # data frame의 각 열을 지칭할 때 cars$ 를 사용
...
#dist<-c(1:10)
#speed<-c(101:110)
attach(cars) # cars data frame을 attach
head(dist/speed)
mean(speed)
mean(dist)
detach(cars) # attach 해제
...
```{r}
write.table(cars, "cars.txt", sep=" " ) #구분자 space, row name 포함
```

-Q5. cars 데이타를 변수 구분자 space, row name이 포함되지 않게 cars1.txt에 저장하시오. 
```{r}
write.table(cars, "cars1.txt",sep=" " , row.name=FALSE) #구분자 space, row name 미포함
```

-Q6. cars 데이타를 변수 구분자 ",", row name이 포함되지 않게 cars1.csv에 저장하시오. 
```{r}
write.csv(cars, "cars1.csv", row.names=FALSE) #구분자 ",", row name 미포함 
```


-Q7. cars1.csv을 읽어오시오. 
```{r}
j_cars_1<- read.csv("cars1.csv") #header 포함, 구분자 ",", row name 포함 
head(j_cars_1)
# read.csv returns data.frame
```

```{r}
j_cars_2<- read.csv("cars1.csv", header=FALSE, skip=1) #header 미포함, 2번째 row 부터 읽기
head(j_cars_2)
```

-Q8. cars.txt을 읽어오시오. 
```{r}
j_cars_3<- read.table("cars.txt",skip=1, sep=" ") #2번째 row 부터 , 구분자 space, row name도 변수로 읽혀짐 
head(j_cars_3)
# read.table also returns data.frame
```

```{r}
j_cars_4<- read.table("cars.txt",skip=1, row.names=1, sep=" ") #첫번째 열을 row name으로 
head(j_cars_4)
```
...
```{r}
j_cars_5<- read.table("cars1.txt", row.names=a[1:50], header=TRUE, sep=" ") #row name을 새로 정해서 
head(j_cars_5)
j_cars_5["a1",]
```
```{r}
